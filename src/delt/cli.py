...
def main(argv):
    ...
    build_info.add_argument("-X", "--disable", nargs="*", help="Names of environment sources to disable")
    build_info.add_argument("--service", help="Specify the CI service being used for the current build")
    build_info.add_argument("--branch", help="Specify the branch for the current build")
    build_info.add_argument("--commit", help="Specify the commit SHA1 for the current build")
    build_info.add_argument("--pull-request", help="Specify the pull request number for the current build if any")
    build_info.add_argument("--tag", help="Specify the tag for the current build if any")
    build_info.add_argument("--include-env", nargs="*", default=[], help="List of environment variables to include that would otherwise be excluded")
    build_info.add_argument("--exclude-env", nargs="*", default=["LS_COLORS", "PS1", "PS2"], help="List of environment variable names to exclude that would otherwise be included")
    ...
    enterprise.add_argument("--project-slug", help="Project slug in the format 'host/owner/name' (eg 'github/delt-io/delt')")
    enterprise.add_argument("--upload-url", default="https://cloudfunctions.net/delt-api", help="Base URL to upload results to")
    ...
    debugging.add_argument("-v", "--version", action="store_true", help="Print the current installed version")
    debugging.add_argument("-d", "--debug", action="store_true", help="Print out debugging information")
    debugging.add_argument("-q", "--quiet", action="store_true", help="Squelch all output to the terminal")
    debugging.add_argument("--no-color", action="store_true", help="Don't use colors for terminal output")
...
def discover_build_info(context):
    ...
    # Attempt to get branch and commit info from git
    if "branch" not in context.build_info:
        context.build_info["branch"] = context.get_output_from_popen("git rev-parse --abbrev-ref HEAD")
    if "commit" not in context.build_info:
        context.build_info["commit"] = context.get_output_from_popen("git rev-parse HEAD")

    for cls in sorted(DataSource.__subclasses__(), key=lambda cls: (cls.priority, cls.name)):
...
def upload_environment(context):
    ...
    context.debug("Build info collected:\n%s" % json.dumps(context.build_info, sort_keys=True, indent=2))
